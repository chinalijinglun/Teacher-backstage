      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <view-input v-if="!scope.row.subjectClass[2]" :text="scope.row.subjectClass[3] || '未填写'"></view-input>
            <view-subject-select v-if="scope.row.subjectClass[2]" :id="scope.row.subjectClass[2]"></view-subject-select>
          </template>
        </el-table-column>
        <el-table-column
          label="年级">
          <template slot-scope="scope">    
            <view-input :text="$GRADE_ENUMS[scope.row.grade]"></view-input>
          </template>
        </el-table-column>
      </el-table>
    </el-row>
