            ...
            label="时间段">
						<template slot-scope="scope">    
							<div class="time-range-contain" v-for="(item, index) in scope.row.timeRanges" :key="index">
								...
								<el-button size="mini" v-if="index < scope.row.timeRanges.length-1" @click="handlerRemove(scope.row.timeRanges, index)">-</el-button>
							</div>
						</template>
          </el-table-column>
        </el-table>
...
export default {
	...
	methods: {
		...
		getForm() {
			...
			return {
				removeLs: this.originData.map(item => item.id),
				addLs
			}
		}
	}
